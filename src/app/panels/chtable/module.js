...
function (angular, app, _, kbn, moment) {
  ...
  var module = angular.module('kibana.panels.chtable', []);
  app.useModule(module);

  module.controller('chtable', function($rootScope, $scope, $modal, $q, $compile, $timeout,
    fields, querySrv, dashboard, filterSrv,clickhouseFilterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Export",
    ...
    $scope.build_search = function(field,value,negate,couldFilter) {
      ...
      filterSrv.set({type:'field',field:field,query:value,mandate:(negate ? 'mustNot':'must')});
    };
    ...
    $scope.get_data = function(segment, query_id, couldSort) {
      ...
      $scope.chclient.query(stmt).then(
        function(response){
          ...
          $scope.data = _.map(results.data, function(hit) {
            var _h = {
                _source: hit,
                kibana: { _source: hit}
              }
          ...
          $scope.current_fields = _.map(results.meta, function(e) {
            return e['name']
          })
